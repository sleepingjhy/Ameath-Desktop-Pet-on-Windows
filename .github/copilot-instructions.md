...
- `pet/music_player.py`：全局音乐播放器（QMediaPlayer + QAudioOutput），负责播放列表、切歌、播放模式、音量与进度信号。
...
- `pet/app_window.py`：前端应用窗口（设置页、音乐页、关于页），支持与右键设置及音乐状态实时双向同步。
...
- 修改音乐播放优先看：`pet/music_player.py`、`pet/menu.py`、`pet/app_window.py`、`main.py`。
- 修改音乐批量删除与勾选交互优先看：`pet/app_window.py`（删除模式提示、勾选框样式、整行点击勾选）。
- 排查内存泄漏优先看：`pet/window.py`、`pet/music_player.py`、`pet/instance_manager.py`、`pet/app_window.py`（信号断连、定时器停止、deleteLater 链路）。
